// Verbessertes Debug-Skript für PDF-Download-Problem in BuildWise-Gebühren
  ...
  try {
    ...
    if (pdfResponse.ok) {
      ...
      if (blob.size > 0) {
        ...
        console.error('❌ PDF-Datei ist leer');
        return false;
      }
    } else {
...
// Funktion zum Testen der API-Basis-URL
function testApiBaseUrl() {
  console.log('🔍 Debug: Teste API-Basis-URL...');
  
  // Suche nach getApiBaseUrl Funktion
  if (window.getApiBaseUrl) {
    const baseUrl = window.getApiBaseUrl();
    console.log('API-Basis-URL:', baseUrl);
  } else {
    console.log('getApiBaseUrl Funktion nicht verfügbar');
  }
  
  // Teste verschiedene URLs
  const testUrls = [
    '/api/v1/buildwise-fees/',
    'http://localhost:8000/api/v1/buildwise-fees/',
    'https://localhost:8000/api/v1/buildwise-fees/'
  ];
...
async function runPDFDownloadDebug() {
  ...
  // 3. Frontend-Komponente debuggen
